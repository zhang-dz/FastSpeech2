...
# 创建保存图的目录
os.makedirs('output/plots_400k', exist_ok=True)
...
plt.title('两个模型训练总损失对比 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('总损失')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/总损失训练对比.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('两个模型验证总损失对比 (前400k步)')
plt.xlabel('验证步数')
plt.ylabel('总损失')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/总损失验证对比.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('原模型训练损失曲线 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('损失值')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/原模型训练损失曲线.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('新模型训练损失曲线 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('损失值')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/新模型训练损失曲线.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('两个模型Mel损失对比 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('Mel损失')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/Mel损失对比.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('两个模型音高损失对比 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('音高损失')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/音高损失对比.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('两个模型能量损失对比 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('能量损失')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/能量损失对比.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('两个模型持续时间损失对比 (前400k步，每100步采样)')
plt.xlabel('训练步数')
plt.ylabel('持续时间损失')
plt.xlim(0, 400000)  # 设置x轴范围为0到400000
plt.legend()
plt.grid(True)
plt.savefig('output/plots_400k/持续时间损失对比.png', dpi=300, bbox_inches='tight')
plt.close()

# 9. 收敛速度对比 - 找到模型达到特定损失阈值的步数（仅考虑400k步内）
def find_convergence_step(steps, losses, threshold):
    for i, loss in enumerate(losses):
        if loss <= threshold:
            return steps[i]
    return None  # 如果在前400k步内没有达到阈值，返回None
...
for threshold in thresholds:
    attn_step = find_convergence_step(attn_train_data['steps'], attn_train_data['total_losses'], threshold)
    new_step = find_convergence_step(new_train_data['steps'], new_train_data['total_losses'], threshold)
    
    attn_convergence_steps.append(attn_step)
    new_convergence_steps.append(new_step)
...
if valid_thresholds:
    ...
    plt.title('两个模型达到特定损失阈值所需的训练步数 (前400k步内)')
    plt.xlabel('损失阈值')
    plt.ylabel('训练步数')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('output/plots_400k/收敛速度对比.png', dpi=300, bbox_inches='tight')
    plt.close()

print("所有图表已保存到 output/plots_400k 目录") 